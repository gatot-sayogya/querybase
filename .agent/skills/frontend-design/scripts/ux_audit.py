...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        has_form = bool(re.search(r'<form\b|<input\b|password|credit[\s-]*card|payment', content, re.IGNORECASE))
        ...
        if nav_items > 7 and not re.search(r'layout|playwright', filename.lower()):
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            dirs[:] = [d for d in dirs if d not in {'node_modules', '.git', 'dist', 'build', '.next', 'designs', 'out', 'playwright-report'}]
            for file in files:
